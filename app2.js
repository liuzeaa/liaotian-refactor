...
app.use((req,res,next){
	if(!req.cookie.user_id){
		...
		model.User.findById(req.cookie.user_id).then((item)=>{
...
io.on('connection',(socket)=>{
	...
	socket.on('send-message',(obj)=>{
		...
		redis.lpush(obj.to+'-'+obj.from,JSON.string(Object.assign({},obj,{type:'receive',timeStamp:moment.now().read:false})));
		if(clients[obj.to]){
			clients[obj.to].emit('reveive-message'.obj);
		}
	});
	socket.on('send-request',(obj)=>{
		console.log('i am send request')
		...
		model.Request.findById(obj.id).then((item)=>{
			item.update({
				...
				model.Relation.create({
					...
					if(clients[obj.to]){
						clients[obj.to].emit('reveive-request'.obj);
					}
				})
			})
		})
